...
# --- SSR Function ---
def ssr(params):
    λ, log_β, δ, log_p, c, r, roh = params
    β = 10 ** log_β
    p = 10 ** log_p
    ...
    t1 = [0, 24, 26]
    y1 = odeint(base_model, y0, t1, args=(λ, β, δ, p, c, r, roh))
    ...
    y2 = odeint(base_model, y2_init, t2, args=(λ, β, δ, p, c, r, roh))
   ...
   # total_ssr +=  np.nansum(((y2[1, 3]) - (V_data_list[0])) ** 2)  # t=26.04167
    ...
    for i in range(len(T_data_list[0])):
        if not np.isnan(T_data_list[0][i]):
            total_ssr += np.nansum((np.log10(T_data_list[2][i]) - np.log10(y2[3, 0] + y2[3,1])) ** 2)  # t=28
    ...
    # Segment 3: 28–31
    t3 = [28, 31]
    y3 = odeint(base_model, y3_init, t3, args=(λ, β, δ, p, c, r, roh))
    ...
    y4 = odeint(base_model, y4_init, t4, args=(λ, β, δ, p,c, r, roh))
...
initial_guess = [
    0.1004,                  # λ (growth rate of uninfected tumor cells)
...
λ_fit, β_fit_log, δ_fit, p_fit_log, c_fit, r_fit, roh_fit = result.x
β_fit = 10 ** β_fit_log
p_fit = 10 ** p_fit_log
...
total_tumor = np.sum(odeint(base_model, [1.76, 0, 0, 0], all_times,
                            args=(λ_fit, β_fit, δ_fit, p_fit, c_fit, r_fit, roh_fit))[:, :3], axis=1)
...
total_tumor = np.sum(odeint(base_model, [1.76, 0, 0, 0], all_times,
                            args=(λ_fit, β_fit, r_fit, δ_fit, p_fit, c_fit))[:, :3], axis=1)
